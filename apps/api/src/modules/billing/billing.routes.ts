...
// Mock Invoices Data Store (since we don't have a Prisma model for Invoice yet in the provided schema)
...
billingRouter.get('/invoices', authenticate, async (req: Request, res: Response, next: NextFunction) => {
    try {
        return res.json({ invoices: MOCK_INVOICES });
    } catch (e) {
        return next(e);
    }
});
...
billingRouter.patch('/invoices/:id', authenticate, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        return res.json({ invoice: MOCK_INVOICES[invoiceIndex] });
    } catch (e) {
        return next(e);
    }
});
